...
def login_system():
    ...
    def view_sales_by_store():
        ...
        while True:
            ...
            print(f"Offset: {offset}, Records fetched in this batch: {num_records}")
    ...
    def view_sales_by_invoice():
        while True:
            invoice_id = input(
                "Enter the invoice Number to view sales information or 'q' to return to the previous menu: ")
            if invoice_id.lower() == 'q':
                break

            query = """
            SELECT o.`Order_Number`, 
            SUM(p.`Product_Unit_Price` * oi.`Purchased_Quantity`) AS Total_Sales_Amount
            FROM `order` o
            ...
            if not results:
                print("No sales records found for the given invoice Number.")
                continue
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.ln(10)
        def invoice_body(self, table_data, total_price, total_cost):
            self.set_font('Arial', '', 12)
            ...
            self.ln(10)
            self.set_font('Arial', 'B', 12)
            self.cell(0, 10, f'Total Price (without GST): ${total_price:.2f}', 0, 1)
            self.ln(10)
        ...
        while True:
            ...
            try:
                ...
                for row in results:
                    ...
                    total_cost += unit_price * quantity * 0.96
                ...
                print("--------------------------------------------------")
                print("Total Price (without GST): ${:<10.2f}".format(total_price))
                ...
                print("You can press ENTER to continue")
                ...
                if pdf_file:
                    ...
                    pdf.add_page()
                    pdf.invoice_body(table_data, total_price, total_cost)
                    pdf.output(pdf_file)
    ...
    def view_all_sales():
        ...
        offset = 0
        total_fetched = 0
        order_by = "`Product Name`"  # Default sorting

        while True:
            # Display the submenu if order_by is not set
            if offset == 0:
                print(
                ...
                if choice == '1':
                    ...
                    order_by = "`Product Unit Price` ASC"
                elif choice == '4':
                    order_by = "`Product Unit Price` DESC"
                elif choice == '5':
            ...
            # Fetch and display records based on selected order
            cursor.execute(f"""
            SELECT 
                o.Order_Number,
                o.Order_Date,
                ...
                c.Cust_Name,
                c.Cust_City,
                c.Cust_State AS `Cust State`,
                c.Cust_State_Code AS `Cust State Code`,
                c.Cust_Zip_Code,
                c.Cust_Country,
                c.Cust_Continent,
                c.Cust_Birthday,
                ...
                psc.Subcategory_Name AS `Product Subcategory`,
                oi.Purchased_Quantity,
                o.Order_Date,
                o.Delivery_Date
                FROM 
                ...
                JOIN
                    `store location` sl ON s.Store_Location_Id = sl.Store_Location_Id
                JOIN
                    `category` pc ON p.Product_Category_Id = pc.Category_Id
                LEFT JOIN
                    `subcategory` psc ON p.Product_Category_Id = psc.Product_Category_Id
                ORDER BY 
                    {order_by}
                LIMIT 
                    100 OFFSET {offset};
            ...
            column_names = [
                ...
                "Product Subcategory", "Purchased Quantity", "Order Date", "Delivery Date",
            ...
            # Ask user if they want to view the next 100 records
            more = input("Do you want to view the next 100 records? \n1. Yes\n2. No\n: ")
            if more == '1':
                offset += 100
            else:
    ...
    def add_new_order():
        ...
        cursor.execute("SELECT COUNT(*) FROM `order` WHERE Order_Number = %s", (order_number,))
        if cursor.fetchone()[0] > 0:
            print("Error: Order number already exists.")
            return
        ...
        try:
            cursor.execute("""
                INSERT INTO `order` (Order_Number, Store_Id, CUST_Id, Delivery_Date, Order_Date)
                VALUES (%s, %s, %s, %s, %s)
            """, (order_number, store_id, cust_id, delivery_date, order_date))
    ...
    def modify_existing_order():
        order_id = input("Enter the order ID to modify: ")
        ...
        order_number_input = input(f"Enter new order number (current: {order[1]}): ")
        ...
        order_number = order_number_input if order_number_input.strip() else order[1]
        ...
        try:
            cursor.execute("""
                UPDATE `order`
                SET Order_Number = %s, Store_Id = %s, CUST_Id = %s, Delivery_Date = %s, Order_Date = %s
                WHERE Order_Id = %s
            """, (order_number, store_id, cust_id, delivery_date, order_date, order_id))
